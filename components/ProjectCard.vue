<template>
  ...
  <div
    class="group relative block overflow-hidden rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500"
    role="button"
    tabindex="0"
    @click="$emit('preview', project)"
    @keydown.enter.prevent="$emit('preview', project)"
  ...
  >
    ...
    <div class="relative aspect-[16/9] w-full bg-gray-200 dark:bg-gray-800">
      ...
      <!-- Top row: badges + menu -->
      <div
        class="absolute inset-x-0 top-0 z-20 flex items-start justify-between p-3"
      >
        <!-- Badges -->
        <div class="flex items-center gap-2">
          ...
          <span
            v-if="project.author"
            class="rounded-md bg-black/60 px-2 py-0.5 text-[10px] font-medium text-white backdrop-blur"
      ...
      <!-- Gradient overlay -->
      <div
        class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-black/0"
      />
      ...
      <div class="absolute inset-x-0 bottom-0 z-20 p-4">
        ...
        <!-- Title -->
        <h3
          class="line-clamp-2 text-lg sm:text-xl font-extrabold leading-snug text-white drop-shadow-md"
        >
          {{ project.title }}
        </h3>
        ...
        <div class="mt-2 flex items-center justify-between">
          ...
          <span
            class="inline-flex items-center gap-2 rounded-lg backdrop-blur-lg px-3 py-1.5 text-xs font-semibold text-white transition-all duration-200 group-hover:translate-y-[-1px] group-hover:shadow-md"
          >
