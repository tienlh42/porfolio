<template>
    <Teleport to="body">
        <transition name="fade">
            <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center p-4 " role="dialog"
                aria-modal="true" @keydown.esc.prevent="close">
                <!-- Backdrop -->
                ...
                <transition name="pop">
                    <div
                        ...
                        <div class="relative aspect-[16/9] w-full">
                            <img v-if="project?.thumbnail_image" :src="project.thumbnail_image" alt=""
                            ...
                            <div class="absolute bottom-3 left-3 right-3">
                                ...
                                <div class="mt-1 flex items-center gap-3 text-xs text-white/80">
                                    <span v-if="project?.duration" class="rounded bg-white/10 px-2 py-0.5">⏱ {{
                                        project.duration }}</span>
                                    <span v-if="project?.author" class="rounded bg-white/10 px-2 py-0.5">👤 {{
                                        project.author }}</span>
                                    <span v-if="project?.rating" class="rounded bg-white/10 px-2 py-0.5">★ {{
                        ...
                        <div class="p-5 ">
                            ...
                            <ClientOnly>
                                <component v-if="galleryImages.length" :is="$LightGallery"
                                    :plugins="[$lgZoom, $lgThumbnail]" :speed="400" :download="false" :closable="true"
                                    :thumbnail="true" class="mt-5">
                                    <!-- Mỗi ảnh là 1 anchor; data-src là ảnh lớn, img là thumbnail -->
                                    <a v-for="(src, i) in galleryImages" :key="i" :data-src="src"
                                        :data-sub-html="project?.title" class="inline-block mr-3 mb-3">
                                        <img :src="src" alt=""
                            ...
                            <div class="mt-6 flex items-center gap-3">
                                <NuxtLink v-if="project?.id !== undefined" :to="`/projects/${project.id}`"
